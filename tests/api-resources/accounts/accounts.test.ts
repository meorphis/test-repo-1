...
describe('resource accounts', () => {
  test('accountGet: only required params', async () => {
    const responsePromise = meorphisTest42.accounts.accountGet({ 'X-Publishable-Key': 'string' });
  ...
  test('accountGet: required and optional params', async () => {
    const response = await meorphisTest42.accounts.accountGet({ 'X-Publishable-Key': 'string' });
  });
});
