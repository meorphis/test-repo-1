...
import * as Core from 'meorphis-test-9/core';
import { APIResource } from 'meorphis-test-9/resource';
import { isRequestOptions } from 'meorphis-test-9/core';
import * as PetsAPI from 'meorphis-test-9/resources/pets';

export class Pets extends APIResource {
  /**
   * Create a pet
   */
  create(body: PetCreateParams, options?: Core.RequestOptions): Core.APIPromise<void> {
    return this._client.post('/pets', { body, ...options, headers: { Accept: '*/*', ...options?.headers } });
  }
